...
export default function AgendaPage() {
  const { user } = useAuth()
  const { leads, visits, addVisit, updateVisit, removeVisit, updateLead, addInteraction, corretores, userName } = useLocacao()
  ...
  const [editing, setEditing] = useState<LocacaoVisit | null>(null)
  const [delVisit, setDelVisit] = useState<LocacaoVisit | null>(null)
  ...
  function openEditar(v: LocacaoVisit) {
    setEditing(v)
    setData(v.data)
    setHora(v.hora)
  ...
  function confirmEditar(e: React.FormEvent) {
    ...
    addInteraction(editing.leadId, { corretor: userName(editing.corretorId), texto: `Visita de locação reagendada para ${data.split("-").reverse().join("/")} às ${hora}.` })
  ...
  function confirmDelete() {
    ...
    addInteraction(delVisit.leadId, { corretor: userName(delVisit.corretorId), texto: `Visita de locação de ${delVisit.data.split("-").reverse().join("/")} às ${delVisit.hora} cancelada.` })
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <PageHeading title="Agenda de Visitas de Locação" subtitle="Visualize e agende visitas de locação vinculadas aos leads." badge="Visitas" />
                  ...
                  return (
                    <div key={v.id} className="rounded-lg border border-border p-3">
                      <div className="flex items-center justify-between">
                        ...
                        {lead && (
                          <Link href={`/locacao/${lead.id}`} className="flex items-center gap-1 text-xs font-medium text-primary hover:underline">
